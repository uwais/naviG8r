...
- `GET /shipments`, `GET /shipments/:id`, and **`GET /shipments/:id/tracking`** require `Authorization: Bearer`. Responses include shipments for your **CUSTOMER** org (`customerOrgId` match only) **or** shipments whose **`bookedByPhone`** / **`bookedByUserId`** matches your user. Org `displayName` is not used for authorization.
