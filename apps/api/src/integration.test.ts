...
test("integration idempotency key cannot be reused for a different external load", async () => {
  await withEnv({ AUTH_SECRET: "test-secret-min-16-chars!!", PAYMENT_PROVIDER: undefined }, async () => {
    const store = createStore();
    seedOpenTrip(store);
    const admin = registerCustomerOrgAdmin(store, {
    ...
    const base = {
      ...
      pickup: GURGAON,
      drop: JAIPUR,
      idempotencyKey: "idem-erp-001",
    };

    const first = await createIntegrationLoad(store, ctx, {
      ...base,
      externalLoadId: "ERP-IDEM-001",
    });
    assert.equal(first.created, true);

    await assert.rejects(
      () => createIntegrationLoad(store, ctx, { ...base, externalLoadId: "ERP-IDEM-002" }),
      /integration_idempotency_conflict/,
    );
  });
});

test("integration event cursor returns the next events without skipping older rows", async () => {
  await withEnv({ AUTH_SECRET: "test-secret-min-16-chars!!", PAYMENT_PROVIDER: undefined }, async () => {
    const store = createStore();
    const { driver } = seedOpenTrip(store);
    const admin = registerCustomerOrgAdmin(store, {
      fullName: "ERP Cursor Admin",
      phone: "9111001192",
      orgDisplayName: "ERP Cursor Co",
    });
    const { token } = createIntegrationApiKey(store, admin.org.id);
    updateIntegrationConnection(store, admin.org.id, { paymentPolicy: "erp_preauthorized" });
    const ctx = resolveIntegrationAuth(store, { bearerToken: token });
    const created = await createIntegrationLoad(store, ctx, {
      externalLoadId: "ERP-CURSOR-001",
      weightKg: 100,
      pickupAddress: "Warehouse A, Gurugram",
      dropAddress: "Plant B, Jaipur",
      pickup: GURGAON,
    ...
    acceptCarrierShipment(store, { shipmentId: created.shipment.id, userId: driver.user.id });
    const events = listIntegrationEvents(store, admin.org.id, {});
    assert.ok(events.length >= 4);

    const afterFirst = listIntegrationEvents(store, admin.org.id, {
      sinceEventId: events[0]!.id,
      limit: 2,
    });
    assert.deepEqual(afterFirst.map((e) => e.id), events.slice(1, 3).map((e) => e.id));
  });
});
