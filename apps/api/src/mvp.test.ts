import { bookTestShipment, registerCompliantCarrier, deliverTestShipment, runTestPayoutBatch } from "../test/fixtures.ts";
...
import {  acceptCarrierShipment,  publishAnchorTrip,  } from "./services.ts";
...
test("vertical slice: publish trip -> book (capture) -> POD -> ledger -> weekly batch pays after cutoff", async () => {
  ...
  const out = await deliverTestShipment(store, { shipmentId: shipment.id, podAtUtcMs });
  ...
  const before = await runTestPayoutBatch(store, { nowUtcMs: expected.payoutBatchCutoffUtcMs - 1 });
  ...
  const after = await runTestPayoutBatch(store, { nowUtcMs: expected.payoutBatchCutoffUtcMs });
